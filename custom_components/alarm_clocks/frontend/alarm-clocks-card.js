...
`;let qt=class extends $t{constructor(){super(...arguments),this.days=[],this.compact=!1}render(){if(!this.days.length)return ot;const t=Mt(this.hass),e=Pt(this.hass),i=Lt(t,"short"),s=Lt(t,"long"),n=Lt(t,"narrow");return st`
    ...
    `}_renderSegment(t,e,i,s){const n=Pt(this.hass);return st`
      <div class="segment">
        ...
        <div
          ...
          @pointerdown=${e=>this._startDrag(e,t)}
          @pointermove=${e=>this._onDrag(e,t)}
    ...
    `}_step(t,e){if(this.disabled)return;let{hours:i,minutes:s}=this;if("hours"===t)i=(i+e+24)%24;else{const t=Math.max(1,Math.round(this.minuteStep));s=((Math.round(s/t)*t+e*t)%60+60)%60}this.hours=i,this.minutes=s,this.dispatchEvent(new CustomEvent("time-changed",{detail:{hours:i,minutes:s},bubbles:!0,composed:!0}))}_startHold(t,e,i){this.disabled||(t.preventDefault(),t.currentTarget.setPointerCapture?.(t.pointerId),this._step(e,i),this._repeatDelay=180,this._holdTimer=window.setTimeout(()=>{const t=()=>{this._step(e,i),this._repeatDelay=Math.max(60,this._repeatDelay-12),this._repeatTimer=window.setTimeout(t,this._repeatDelay)};t()},450))}_startDrag(t,e){this.disabled||(t.currentTarget.setPointerCapture?.(t.pointerId),this._dragOrigin={y:t.clientY,hours:this.hours,minutes:this.minutes})}_onDrag(t,e){if(!this._dragOrigin)return;t.preventDefault();const i=Math.trunc((this._dragOrigin.y-t.clientY)/18);if(0!==i){this._dragOrigin.y-=18*i,this._step(e,i>0?1:-1);for(let t=1;t<Math.abs(i);t+=1)this._step(e,i>0?1:-1)}}_onWheel(t,e){this.disabled||0===t.deltaY||(t.preventDefault(),this._step(e,t.deltaY<0?1:-1))}_onKeyDown(t,e){"ArrowUp"===t.key?(t.preventDefault(),this._step(e,1)):"ArrowDown"===t.key&&(t.preventDefault(),this._step(e,-1))}};Vt.styles=[Wt,Bt,y`
      .stepper {
        display: flex;
        align-items: center;
        gap: 2px;
    ...
    `:ot}_renderHero(t,e){const i=Mt(this._hass),s=Boolean(t.entities.alarmTime),n=this._pendingTime??t.alarmTime;return n?st`
      <div class="hero">
    ...
    `:st`
      ...
      `}_renderRow(t,e){const i=Mt(this._hass),s=t.alarmTime?`${String(t.alarmTime.hours).padStart(2,"0")}:${String(t.alarmTime.minutes).padStart(2,"0")}`:e("label.no_time");return st`
